        ...
        <div class="row">
            ...
            $(document).ready(function() {
                    ...
                    Morris.Bar({
                        ...
                        data: [
                            { y: 'Jan',  a: "{{ $budget_collection->get(0)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(0)?->sum('total_expense') ?? 0 }}" },
                            { y: 'Feb',  a: "{{ $budget_collection->get(1)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(1)?->sum('total_expense') ?? 0 }}" },
                            { y: 'Mar',  a: "{{ $budget_collection->get(2)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(2)?->sum('total_expense') ?? 0 }}" },
                            { y: 'Apr',  a: "{{ $budget_collection->get(3)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(3)?->sum('total_expense') ?? 0 }}"},
                            { y: 'May',  a: "{{ $budget_collection->get(4)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(4)?->sum('total_expense') ?? 0 }}" },
                            { y: 'Jun',  a: "{{ $budget_collection->get(5)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(5)?->sum('total_expense') ?? 0 }}"},
                            { y: 'Jul',  a: "{{ $budget_collection->get(6)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(6)?->sum('total_expense') ?? 0 }}" },
                            { y: 'Aug',  a: "{{ $budget_collection->get(7)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(7)?->sum('total_expense') ?? 0 }}"},
                            { y: 'Sept', a: "{{ $budget_collection->get(8)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(8)?->sum('total_expense') ?? 0 }}"},
                            { y: 'Oct',  a: "{{ $budget_collection->get(9)?->sum('total_revenue') ?? 0 }}",  b: "{{ $budget_collection->get(9)?->sum('total_expense') ?? 0 }}"},
                            { y: 'Nov',  a: "{{ $budget_collection->get(10)?->sum('total_revenue') ?? 0 }}", b: "{{ $budget_collection->get(10)?->sum('total_expense') ?? 0 }}"},
                            { y: 'Dec',  a: "{{ $budget_collection->get(11)?->sum('total_revenue') ?? 0 }}", b: "{{ $budget_collection->get(11)?->sum('total_expense') ?? 0 }}"},
                        ],
                        xkey: 'y',
                        ykeys: ['a', 'b'],
                    ...
                    Morris.Line({
                        ...
                        redrawOnParentResize: true,
                        data: [
                            { y: 1,  a: "{{ $invoice_collection->get(0)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(0)?->sum('grand_total') ?? 0 }}" },
                            { y: 2,  a: "{{ $invoice_collection->get(1)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(1)?->sum('grand_total') ?? 0 }}" },
                            { y: 3,  a: "{{ $invoice_collection->get(2)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(2)?->sum('grand_total') ?? 0 }}" },
                            { y: 4,  a: "{{ $invoice_collection->get(3)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(3)?->sum('grand_total') ?? 0 }}"},
                            { y: 5,  a: "{{ $invoice_collection->get(4)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(4)?->sum('grand_total') ?? 0 }}" },
                            { y: 6,  a: "{{ $invoice_collection->get(5)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(5)?->sum('grand_total') ?? 0 }}"},
                            { y: 7,  a: "{{ $invoice_collection->get(6)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(6)?->sum('grand_total') ?? 0 }}" },
                            { y: 8,  a: "{{ $invoice_collection->get(7)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(7)?->sum('grand_total') ?? 0 }}"},
                            { y: 9,  a: "{{ $invoice_collection->get(8)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(8)?->sum('grand_total') ?? 0 }}"},
                            { y: 10, a: "{{ $invoice_collection->get(9)?->sum('grand_total') ?? 0 }}",  b: "{{ $estimates_collection->get(9)?->sum('grand_total') ?? 0 }}"},
                            { y: 11, a: "{{ $invoice_collection->get(10)?->sum('grand_total') ?? 0 }}", b: "{{ $estimates_collection->get(10)?->sum('grand_total') ?? 0 }}"},
                            { y: 12, a: "{{ $invoice_collection->get(11)?->sum('grand_total') ?? 0 }}", b: "{{ $estimates_collection->get(11)?->sum('grand_total') ?? 0 }}"},
                        ],
                        xkey: 'y',
                        ykeys: ['a', 'b'],
